...
hidePreloader();
(function ($) {
    var settings = { inEffect: { opacity: 'show' }, inEffectDuration: 600, stayTime: 3000, text: '', sticky: false, type: 'notice', position: 'top-right', closeText: '', close: null }; var methods = {
        init: function (options) { if (options) { $.extend(settings, options); } }, showToast: function (options) {
            ...
            if (!localSettings.sticky) {
                setTimeout(function () { $().toastmessage('removeToast', toastItemInner, localSettings); }, localSettings.stayTime);
            }
            return toastItemInner;
        }, showNoticeToast: function (message) { var options = { text: message, type: 'notice' }; return $().toastmessage('showToast', options); }, showSuccessToast: function (message) { var options = { text: message, type: 'success' }; return $().toastmessage('showToast', options); }, showErrorToast: function (message) { var options = { text: message, type: 'error' }; return $().toastmessage('showToast', options); }, showWarningToast: function (message) { var options = { text: message, type: 'warning' }; return $().toastmessage('showToast', options); }, removeToast: function (obj, options) {
            obj.animate({ opacity: '0' }, 600, function () {
                obj.parent().animate({ height: '0px' }, 300, function () { obj.parent().remove(); });
            }); if (options && options.close !== null) { options.close(); }
        }
    }; $.fn.toastmessage = function (method) { if (methods[method]) { return methods[method].apply(this, Array.prototype.slice.call(arguments, 1)); } else if (typeof method === 'object' || !method) { return methods.init.apply(this, arguments); } else { $.error('Method ' + method + ' does not exist on jQuery.toastmessage'); } };
})(jQuery);
...
$().toastmessage('showToast', { text: 'Welcome!', sticky: false, position: 'bottom-right', type: 'notice', inEffectDuration: 1000, stayTime: 7000, }); function toast(type, message) { $().toastmessage('showToast', { text: message, sticky: false, position: 'bottom-right', type: this.type, inEffectDuration: 1000, stayTime: 7000, }); }
var x = 0; var msg = ''; var ip; $.get('https://ipinfo.io', function (response) { ip = (response.ip); var city = (response.city); var region = (response.region); var country = (response.country); var loc = (response.loc); var org = (response.org); }, 'jsonp'); function counter() {
    x++; switch (x) { case 1: msg = "Noor Chahal"; break; case 2: msg = 'Playback Singer.'; break; case 3: msg = 'Thank you for visiting my website :)'; break; case 4: msg = 'Be somebody nobody thought you could be.'; break; case 5: msg = 'In the middle of every difficulty lies opportunity.'; break; case 6: msg = 'It is not the mountain we conquer but ourselves.'; break; case 7: msg = 'He who has a why to live can bear almost any how.'; break; case 8: msg = 'Begin anywhere.'; break; case 9: msg = 'Everything you can imagine is real.'; break; case 10: msg = 'You are never too old to set another goal or to dream a new dream.'; break; case 11: msg = 'The really great man is the man who makes every man feel great.'; break; case 12: msg = 'Compassion is language the deaf can hear and the blind can see.'; break; default: x = 1, msg = "Noor Chahal"; }
...
function toggle_div_fun(id) {
    var divelement = document.getElementById(id); if (divelement.style.display == 'block') { $("#sectiontohide").hide("slow"); }
    else { $("#sectiontohide").show("slow", function () { $("#sectiontohide").delay(10000).hide("slow"); }); }
}
(function () {
            ...
            links[i].onclick = function () {
                var element; var href = this.attributes.href.nodeValue.toString(); if (element = document.getElementById(href.substr(1))) {
                    var hop_count = speed / moving_frequency; var getScrollTopDocumentAtBegin = getScrollTopDocument(); var gap = (getScrollTopElement(element) - getScrollTopDocumentAtBegin) / hop_count; for (var i = 1; i <= hop_count; i++) {
                        (function () { var hop_top_position = gap * i; setTimeout(function () { window.scrollTo(0, hop_top_position + getScrollTopDocumentAtBegin); }, moving_frequency * i); })();
                    }
                }
                return false;
            };
    ...
    var getScrollTopElement = function (e) {
        var top = 0; while (e.offsetParent != undefined && e.offsetParent != null) { top += e.offsetTop + (e.clientTop != null ? e.clientTop : 0); e = e.offsetParent; }
        return top;
    }; var getScrollTopDocument = function () { return document.documentElement.scrollTop + document.body.scrollTop; };
})(); $(function () { var selectedClass = ""; $(".filter").click(function () { selectedClass = $(this).attr("data-rel"); $("#gallery").fadeTo(100, 0.1); $("#gallery div").not("." + selectedClass).fadeOut().removeClass('animation'); setTimeout(function () { $("." + selectedClass).fadeIn().addClass('animation'); $("#gallery").fadeTo(300, 1); }, 300); }); }); function check() {
    if (document.getElementById("checkbox7").checked == true) { document.getElementById("message").value += " (Hire Checked)"; }
    else {
        var word = " (Hire Checked)"; var textValue = document.getElementById('message').value; if (textValue.indexOf(word) != -1) { var res = document.getElementById("message").value.replace(word, ""); document.getElementById("message").value = res; }
    }
}
$(document).ready(function () {
    $('#carousel-example-1').hammer().on('swipeleft', function () { $(this).carousel('next'); })
